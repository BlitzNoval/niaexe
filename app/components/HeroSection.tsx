'use client';

import { motion } from 'framer-motion';
import MatrixGlobe from './MatrixGlobe';

export default function HeroSection() {
	return (
		<motion.section
			initial={{ opacity: 0 }}
			animate={{ opacity: 1 }}
			transition={{ duration: 1 }}
			className="h-screen flex items-center justify-center relative overflow-hidden"
		>
			{/* Matrix Globe Background */}
			<MatrixGlobe />

			{/* Content Overlay */}
			<div className="relative z-20 text-center">
				<motion.h1
					initial={{ y: 50, opacity: 0 }}
					animate={{ y: 0, opacity: 1 }}
					transition={{ duration: 0.8, delay: 0.5 }}
					className="text-4xl md:text-7xl font-bold mb-6 font-mono text-green-400 drop-shadow-[0_0_10px_rgba(0,255,65,0.8)]"
				>
					LIAM MOODLEY
				</motion.h1>
				
				<motion.div
					initial={{ y: 50, opacity: 0 }}
					animate={{ y: 0, opacity: 1 }}
					transition={{ duration: 0.8, delay: 0.7 }}
					className="mb-8 px-4 text-center"
				>
					<p className="text-lg md:text-xl text-green-300 mb-4 font-mono">
						&gt; INTERACTIVE WEB DEVELOPER & UI/UX DESIGNER
					</p>
					<p className="text-base md:text-lg text-green-200 font-light font-mono">
						[ CODING_TO_CONNECT.exe ] [ DESIGNING_FOR_HUMANS.dll ]
					</p>
				</motion.div>

				<motion.div
					initial={{ y: 50, opacity: 0 }}
					animate={{ y: 0, opacity: 1 }}
					transition={{ duration: 0.8, delay: 0.9 }}
					className="flex flex-col items-center gap-4 mb-6"
				>
					<div className="flex items-center gap-2 text-green-300 font-mono">
						<span className="text-green-400">&gt;</span>
						<span className="text-sm">LOCATION: JOHANNESBURG_ZA</span>
					</div>

					<a
						href="mailto:niaexedev@gmail.com"
						className="flex items-center gap-2 px-6 py-3 border border-green-400 hover:bg-green-400/10 rounded transition-colors text-sm font-mono text-green-400 hover:text-green-300"
					>
						<span>&gt; INITIATE_CONTACT.exe</span>
					</a>

					<div className="flex gap-4 mt-4">
						<a 
							href="https://github.com/BlitzNoval" 
							target="_blank" 
							rel="noopener noreferrer" 
							className="p-3 border border-green-400/50 hover:border-green-400 hover:bg-green-400/10 rounded transition-colors text-green-400"
						>
							<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
								<path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
							</svg>
						</a>
						<a 
							href="https://www.linkedin.com/in/liammoodley/" 
							target="_blank" 
							rel="noopener noreferrer" 
							className="p-3 border border-green-400/50 hover:border-green-400 hover:bg-green-400/10 rounded transition-colors text-green-400"
						>
							<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
								<path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
							</svg>
						</a>
						<a 
							href="https://www.figma.com/@liammoodley" 
							target="_blank" 
							rel="noopener noreferrer" 
							className="p-3 border border-green-400/50 hover:border-green-400 hover:bg-green-400/10 rounded transition-colors text-green-400"
						>
							<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
								<path d="M15.852 8.981h-4.588V0h4.588c2.476 0 4.49 2.014 4.49 4.49s-2.014 4.491-4.49 4.491zM12.735 7.51h3.117c1.665 0 3.019-1.355 3.019-3.019s-1.354-3.019-3.019-3.019h-3.117V7.51zm0 1.471H8.148c-2.476 0-4.49-2.015-4.49-4.491S5.672 0 8.148 0h4.588v8.981zm-4.587-7.51c-1.665 0-3.019 1.355-3.019 3.019s1.354 3.02 3.019 3.02h3.117V1.471H8.148zm4.587 15.019H8.148c-2.476 0-4.49-2.014-4.49-4.49s2.014-4.49 4.49-4.49h4.588v8.98zM8.148 8.981c-1.665 0-3.019 1.355-3.019 3.019s1.355 3.019 3.019 3.019h3.117v-6.038H8.148zm7.704 0c2.476 0 4.49 2.015 4.49 4.491s-2.014 4.49-4.49 4.49-4.49-2.015-4.49-4.491 2.014-4.491 4.49-4.491zm0 7.51c1.665 0 3.019-1.355 3.019-3.019s-1.355-3.019-3.019-3.019-3.019 1.355-3.019 3.019 1.354 3.019 3.019 3.019zM8.148 24c-2.476 0-4.49-2.015-4.49-4.491s2.014-4.49 4.49-4.49 4.49 2.015 4.49 4.49S10.624 24 8.148 24zm0-7.51c-1.665 0-3.019 1.355-3.019 3.019s1.355 3.019 3.019 3.019 3.019-1.355 3.019-3.019-1.354-3.019-3.019-3.019z"/>
							</svg>
						</a>
					</div>
				</motion.div>

				{/* Matrix-style scroll indicator */}
				<motion.div
					initial={{ opacity: 0 }}
					animate={{ opacity: 1 }}
					transition={{ duration: 0.8, delay: 1.2 }}
					className="absolute bottom-8 left-1/2 transform -translate-x-1/2"
				>
					<div className="font-mono text-green-400 text-xs animate-pulse">
						&gt; SCROLL_DOWN.exe
					</div>
				</motion.div>
			</div>
		</motion.section>
	);
}
